...
python task1.py genome.fasta $ORF task1_output.txt ## TASK 1 COMMAND
...
python task2.py -i task1_output.txt -o task2_output.txt -e $user_enzyme -pm $min_pep -mc $missed_clevage ## TASK 2 COMMAND
...
echo "Charge: $charge"
python task3.py task2_output.txt -m $random_mass_type -z $charge -o task3_output.txt ## TASK 3 COMMAND

echo -e "\nRunning Task 4: This script takes a table that have info of m/z of every peptide sequence and returns multiple tsv files\n"
for i in {1..4}; do
    python task4.py task3_output.txt -m $i ## TASK 4 COMMAND
    echo
done